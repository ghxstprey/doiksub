...
async function syncSettings() {
    ...
    if (
        ...
        !await dsGet("doiksub_cloudSecret") // this has been enabled due to local settings share or some other bug
    ) {
        ...
        showNotification({
            ...
            onClick: () => SettingsRouter.openUserSettings("doiksub_cloud_panel")
        });
        return;
    }
    ...
    try {
        ...
        if (Settings.autoUpdate) {
            ...
            if (Settings.autoUpdateNotification) {
                notify({
                    title: "doiksub has been updated!",
        ...
        notify({
            title: "A doiksub update is available!",
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of doiksub.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (IS_DISCORD_DESKTOP && Settings.winNativeTitleBar && IS_WINDOWS) {
        createAndAppendStyle("doiksub-native-titlebar-style", coreStyleRootNode).textContent = "[class*=titleBar]{display: none!important}";
    }
}, { once: true });